...
\label{sec:Behavior}
The pure Rescorla-Wagner model has fewer parameters with respect to the three adapted Rescorla-Wagner versions proposed, and all the three versions can be reduced to the pure Rescorla-Wagner, each of three models is nested with the pure Rescorla-Wagner. Thus using Likelihood Ratio tests we can test for each alternative model whether it is better than the Rescorla-Wagner.({\color{red}insert reference})Likelihood ratio test (LRT) is formalize as the ratio of the likelihood functions of the models to compare: 
\begin{equation}
