...
In the previous sessions we presented the Q learning-forgetting model to parameterize the learning functions. Our main questions are: do assembly-pairs predict the reward and compute prediction error? and do different assembly-pair types compute different component of prediction error? and more specifically which are the assembly-pair types computing the evaluation component of the prediction error, parameterized in reinforcement learning model? On this purpose, we focus on the uncertainty $\alpha(t)$ and the prediction error $\delta(t)$.
\begin{figure}
    \centering
