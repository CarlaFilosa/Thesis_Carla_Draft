...
\begin{figure}
    \centering
    \includegraphics[scale=0.37]{figures/RewardDoyaRLSUM.png}
...
Based on broad knowledge on reward prediction signals, we assume that if the assembly-pairs activity codes for the reward prediction we expect intense activity in the CS+ window when the animal was able to predict the reward (see figure \ref{fig:RewDoyaSum} A. middle); whereas the animal was uncertain to get the reward we expect few assembly-pair activation in CS+ window (see figure \ref{fig:RewDoyaSum} A. top). In other words we expect the assembly-pair activity in CS window to anti-correlate with the uncertainty $\alpha_L(t)$. Indeed, the uncertainty is high at the beginning of the task and goes down at the end of the original phase, to raise up again at the beginning of the reversal phase. 
\begin{figure}
    \centering
