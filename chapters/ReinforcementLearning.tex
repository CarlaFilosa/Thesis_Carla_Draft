...
Since the majority if the inter-regional pairs follow a Poisson distribution (see figure \ref{fig:DistributionEx}), we selected these pairs and built two generalized linear models with logarithmic link function, tailored for Poisson distributed data.
...
$E (\mathbf {Y} )={\boldsymbol {\mu }}=g^{-1}(\mathbf {X} {\boldsymbol {\beta }})$ where $g$ is the link function, logarithmic in our case. The two regression models used can be written as follow:
\begin{equation}
    log(\mu_t)=\beta_0+\beta\cdot\alpha(t)
    \label{eq:regrAlpha}
\end{equation}
\begin{equation}
