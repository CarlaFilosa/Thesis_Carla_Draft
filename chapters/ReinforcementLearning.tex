...
\label{sec:Behavior}
The pure Rescorla-Wagner model has fewer parameters with respect to the three adapted Rescorla-Wagner versions proposed, and all the three version can be reduced to the pure Rescorla-Wagner. We compared the pure Rescorla-Wagner with each of the proposed models through separated Likelihood Ratio tests ({\color{red}insert reference}) for nested models. Likelihood ratio test (LRT) is formalize as the ratio of the likelihood functions of the models to compare: 
\begin{equation}
LRT = -2 \log (\frac{\mathcal{L}_s(\hat{\theta})}{\mathcal{L}_g(\hat{\theta})})
\label{eq:LRT}
\end{equation}
The simpler model (s) has fewer parameters than the general model (g).
Doing so, in each test we tested whether the addition of one parameter to the pure Rescorla-Wagner model was needed and brought an improvement. The tests were conducted separately for each animal.
{\color{red}add discussion on likelihood ratio, nested model}The best model was evaluated using the Bayesian Information Criterion, that is formally defined as (\cite{Schwarz})
%{\displaystyle \mathrm {BIC} =\ln(n)k-2\ln({\widehat {L}}).\ } {\displaystyle \mathrm {BIC} =\ln(n)k-2\ln({\widehat {L}}).\ }
where

%{\displaystyle {\hat {L}}} \hat L = the maximized value of the likelihood function of the model {\displaystyle M} M, i.e. {\displaystyle {\hat {L}}=p(x\mid {\widehat {\theta }},M)} {\displaystyle {\hat {L}}=p(x\mid {\widehat {\theta }},M)}, where {\displaystyle {\widehat {\theta }}} {\displaystyle {\widehat {\theta }}} are the parameter values that maximize the likelihood function;
%{\displaystyle x} x = the observed data;
%{\displaystyle n} n = the number of data points in {\displaystyle x} x, the number of observations, or equivalently, the sample size;
%{\displaystyle k} k = the number of parameters estimated by the model. For example, in multiple linear regression, the estimated parameters are the intercept, the {\displaystyle q} q slope parameters, and the constant variance of the errors; thus, {\displaystyle k=q+2} {\displaystyle k=q+2}.
\begin{figure}
\centering
