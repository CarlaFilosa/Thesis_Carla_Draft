...
In fig.\ref{fig:SPN_Ex} two examples of assembly-pairs resulting significant for Friedman's test: on top a SPN+DAN pair, for which the post-hoc analysis revealed a significant difference between the baseline and the CS+ cont. groups. Interestingly we observe in CS+ cont. window less assembly-activity in the first trials and intense assembly-activity in the last trials. In the first trials we assume that the rule of the experiment was not clear to the mouse, while it has to be learnt at the end of the phase,  . The trial by trial assembly-pair activity is shown, 
\begin{figure}
    \centering
