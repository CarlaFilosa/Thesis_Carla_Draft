...
In the previous session we studied the assembly-pairs task related response in Hit trials, namely trials in which the odor was rewarded and the mouse went for it. We pruned assembly-pairs with significant task-related activation, in specific moment of the trial, using the Friedman test. In similar way we analyse the assembly-pairs task-related response for False Alarm (unrewarded odor, the mouse went for reward) and Correct Rejection (unrewarded odor, the mouse sat quiet) trials.\\
\begin{figure}
     \centering
 ...
 \end{figure}
 SPN-DAN assembly-pairs show similar task-related patterns in Hit trials (fig.\ref{fig:HeatPairsDan}, boxes A.1 and A.2), False Alarm trials (fig.\ref{fig:HeatFAPairsDan}, boxes A.1 and A.2), and Correct Rejection trials (fig.\ref{fig:HeatCRPairsDan}, boxes A.1 and A.2), however in False Alarm trials the activation is delayed, as intuitable: the animal is in fact unsure about the stimulus received and performs wrong, while in correct trials, both Hit and Correct Rejection the animal is more confident\footnote{here a distinction between the early and the final stage of the task would be due, we leave this discussion to the next chapter.} about the output will be received, then, being able to predict the reward (or the absence of reward), reacts early to the conditioned stimulus. We assume SPN+DAN assembly-pairs coding for Reward Prediction and we prove this hypothesis in the next chapter.\\FSN-DAN assembly-pairs patterns are more diversified among trials types. In Hit trials they are characterized by an early (CS+) response and a phasic response on retrieval (US), in False Alarm trials remains the activation to the stimulus, unrewarded in this case (CS-), there is no excitatory activation in the window in which we supposed the animal expected the reward (3rd Lick), and interestingly a good portion of FSN-DAN assembly-pairs show inhibitory activity in the window preceding the expected reward (CS- cont.) and in the expected reward window, as it would have been suggested by a coding of the frustration for realizing there will not be a reward.
